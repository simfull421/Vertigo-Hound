...
🧟‍♂️ 3. 대규모 다각도 추적을 위한 엔진 (FollowerEntity & RVO)
수십, 수백 마리의 적이 플레이어를 다양한 각도에서 포위하며 쫓아오게 만들려면 성능 최적화와 충돌 방지가 관건입니다.

초고속 이동 로직 (FollowerEntity): 과거의 AIPath 대신 ECS(Entity Component System) 기반의 FollowerEntity를 사용하세요. 메인 스레드 부하 없이 수백 기의 AI 병렬 처리가 가능합니다. (Burst 컴파일러 패키지 설치 필수)
